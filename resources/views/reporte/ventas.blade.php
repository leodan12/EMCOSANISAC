...
<script type="text/javascript">
    ...
    function drawChart() {
        var data = google.visualization.arrayToDataTable([
            ['Fecha', 'Ventas'], 
            @foreach($ventas as $reg)['{{$reg->fecha}}', {{ $reg->ingreso  }}],
            @endforeach
 
        ]);

        var options = {
            title: 'Nuestras Ventas ',
            hAxis: {
                title: 'Fechas',
                titleTextStyle: {
                    color: '#333'
    ...
    <div class="col-sm-4">
        <div class="card" style="background-color: skyblue; height:190px;">
            <div class="card-body">
                <div class="row">
                    <div class="col-8">
                        <h5 class="card-title">Ingresos del Dia </h5>
                        <h2>S/<span id="ingresosdeldia"> </span></h2>
                    </div>
                ...
                <div class="row">
                    <div class="col-6" style="text-align: left;">
                        <h6>Esta Semana</h6>
                        <h4>S/<span id="ingresosdelasemana"></span></h4>
                    </div>
                    <div class="col-6" style="text-align: right;">
                        <h6>Este Mes</h6>
                        <h4>S/<span id="ingresosdelmes"> </span></h4>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        $.get('/ingresos', function(data) {
            var ingresodia = data['ingresodia'][0].ingresodia;
            var ingresosemana = data['ingresosemana'][0].ingresosemana;
            var ingresomes = data['ingresomes'][0].ingresomes;
            ...
            document.getElementById('ingresosdeldia').innerHTML = ingresodia;
            document.getElementById('ingresosdelasemana').innerHTML = ingresosemana;
            document.getElementById('ingresosdelmes').innerHTML = ingresomes;
    ...
    function llenarTabla(){
        ...
        var fechaF = document.getElementById('fechafinal').value;
         ...
         $.get('/ventasenfecha/' + fechaI + '/' + fechaF, function(data) {
    ...
    function traerDatos(){
        ...
        var fechaF = document.getElementById('fechafinal').value;
         ...
         $.get('/nuestrasventas/' + fechaI + '/' + fechaF, function(data) {
             
              for (var i = 0; i < data.ventas.length; i++) {
                midata.push([ data.ventas[i].fecha ,parseInt( data.ventas[i].ingresos )], );   
             }
             google.charts.setOnLoadCallback(drawChart);  
...
<script>
    $(document).ready(function() {
    var ventasTabla = @json($ventastabla);
